...
IMPLEMENT_DYNAMIC(CVideos, CDialogEx)

CVideos::CVideos(CWnd* pParent /*=NULL*/)
...
void CVideos::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(CVideos, CDialogEx)
	ON_BN_CLICKED(IDOK, &CVideos::OnOK)
	ON_BN_CLICKED(IDCANCEL, &CVideos::OnCancel)
END_MESSAGE_MAP()
...
int CVideos::InitProgram()
{
	...
	{
		CString ffp;
		ffp.Format(_T("%d"), hWnd);

		SetEnvironmentVariable(_T("SDL_WINDOWID"), (LPCWSTR)(LPCTSTR)ffp);

		//char sdl_var[64];
		//sprintf_s(sdl_var, "SDL_WINDOWID=%d", hWnd);    //主窗口句柄      //这里一定不能有空格SDL_WINDOWID=%d"
		//SDL_putenv(sdl_var);
...
int CVideos::InitVariable()
{
	...
	this->GetWindowRect(&m_Picture_rect);
...
int CVideos::Open_codec_context(int * stream_idx, AVFormatContext * fmt_ctx, enum AVMediaType type)
{
	int ret;
...
int CVideos::stream_component_open(StreamState *m_streamstate, int stream_index)
{
	AVFormatContext *ic = m_streamstate->pFormatCtx;
	SDL_AudioSpec wanted_spec, spec;

	if ((unsigned)stream_index < 0 || (unsigned)stream_index >= ic->nb_streams)
	{
		return -1;
	}
...
int CVideos::stream_component_close(StreamState *m_streamstate, int stream_index)
{
	AVFormatContext *ic = m_streamstate->pFormatCtx;
...
void CVideos::packet_queue_init(PacketQueue *q)
{
...
int CVideos::packet_queue_put(PacketQueue *q, AVPacket *pkt)
{
	int ret;
...
int CVideos::packet_queue_get(PacketQueue *q, AVPacket *pkt, int queue_type)
{
	AVPacketList *pkt1;
	int ret;
...
void CVideos::packet_queue_abort(PacketQueue *q)
{
	SDL_LockMutex(q->mutex);
...
int CVideos::packet_queue_put_private(PacketQueue *q, AVPacket *pkt)
{
	AVPacketList *pkt1;
...
void CVideos::packet_queue_start(PacketQueue *q)
{
	SDL_LockMutex(q->mutex);
...
void CVideos::packet_queue_destroy(PacketQueue *q)
{
...
void CVideos::packet_queue_flush(PacketQueue *q)
{
...
void CVideos::video_refresh(void * opaque)
{
	...
	if (m_stream_type == 3)
	{
		...
		{
			str.Format(_T("  0%d:0%d  "), h_minute, h_second);
		}
		else if (h_minute < 10 && h_second >= 10)
		{
			str.Format(_T("  0%d:%d  "), h_minute, h_second);
		}
		else if (h_minute >= 10 && h_second < 10)
		{
			str.Format(_T("  %d:0%d  "), h_minute, h_second);
		}
		else
		{
			str.Format(_T("  %d:%d  "), h_minute, h_second);
		}
		if (m_Isstop) //如果已经停止
		{
...
int CVideos::audio_decode_frame(StreamState *m_streamstate, uint8_t *audio_buf, int buf_size, double *pts_ptr)
{
...
int CVideos::queue_picture(StreamState *m_streamstate, AVFrame *pFrame, double pts)
{
...
void CVideos::video_display(StreamState *m_streamstate)
{
	SDL_Rect rect;
	VideoPicture *vp;
...
void CVideos::alloc_picture(void *userdata)
{
...
int CVideos::video_open(StreamState *m_streamstate)
{
...
int CVideos::read_thread(LPVOID lpParam)
{
	CVideos * pDlg = (CVideos *)lpParam;
	ASSERT(pDlg);
...
int CVideos::refresh_thread(LPVOID lpParam)
{
	CVideos * pDlg = (CVideos *)lpParam;
	ASSERT(pDlg);
...
void CVideos::audio_callback(void * userdata, unsigned char *stream, int len)
{
	CVideos * pDlg = (CVideos *)userdata;
	ASSERT(pDlg);
	int len1, audio_size;
	double pts;
...
int CVideos::video_thread(void *arg)
{
	CVideos * pDlg = (CVideos *)arg;
	ASSERT(pDlg);
...
void CVideos::OnStartMovies(CString Path)
{
	//////////////////////////////////////////////////////////////////////////
	...
	//T2CA    将宽字符集（Unicode）转化为多字符集（ASCII）
	//A2W     将多字符集（ASCII）转化为宽字符集（Unicode）
